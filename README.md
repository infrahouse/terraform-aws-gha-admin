...
| <a name="input_name_prefix"></a> [name\_prefix](#input\_name\_prefix) | Optional disambiguator inserted into all role names (admin, github, state-manager).<br/>Set this when two environments share an AWS account — most commonly when multiple<br/>environments write their state-manager role to the same tfstates account. Leave empty<br/>for the single-environment-per-account default; existing role names are preserved.<br/>Positioned left of repo\_name so it survives truncation to IAM's 64-char limit. | `string` | `""` | no |
